        ...
        this.signChallengeActivityResultLauncher = registerForActivityResult<Intent, ActivityResult>(
            ...
            try {
                if (result.resultCode == RESULT_OK) {
                    ...
                    if (simaIntent == null) {
                        HybridSima.pendingPromise.reject(Throwable("empty-response"))
                    ...
                    if (status == null || status != "success") {
                        HybridSima.pendingPromise.reject(Throwable(message))
                        return@registerForActivityResult
                    }
                    ...
                    if (s.verify(signatureBytes)) {
                        ...
                        HybridSima.pendingPromise.resolve(response)

                    } else {
                        HybridSima.pendingPromise.reject(Throwable("signature-verification-error"))

                    }
                } else if (result.resultCode == RESULT_CANCELED) {
                    HybridSima.pendingPromise.reject(Throwable("operation-canceled"))

                }
            } catch (e: Exception) {
                e.printStackTrace()
                HybridSima.pendingPromise.reject(Throwable("parse-result-error"))
            }
            finish()
        }
        signChallenge()
        ...
        try {
            ...
            if (intent == null) {
                ...
                try {
                    ...
                    if (EXTRA_USER_CODE_VALUE.isNotEmpty()) {
